...
export const PinContainer = ({
  ...
  return (
    ...
    >
      ...
      {title && href && <PinPerspective title={title} href={href} />}
    </div>
  );
};
  ...
  return (
    <motion.div className="pointer-events-none w-full h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      <div className="w-full h-full -mt-7 flex-none inset-0">
        ...
        {["20", "40"].map((height, index) => (
          <motion.div
            key={index}
            className={`absolute right-1/2 bottom-1/2 bg-gradient-to-b from-transparent to-cyan-500 translate-y-[14px] w-px h-${height} group-hover/pin:h-40 ${
              index === 1 ? "" : "blur-[2px]"
            }`}
          />
        ))}
